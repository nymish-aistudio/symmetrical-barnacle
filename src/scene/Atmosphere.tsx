...
export function Atmosphere() {
  ...
  const c = useMemo(() => ({ cold: new Color('#0e131c'), warm: new Color('#1a130d'), neutral: new Color('#0f1219'), out: new Color() }), []);
  useEffect(() => {
  ...
  useFrame(() => {
    ...
    if (fog) { fog.color.copy(c.out); fog.density = lerp(0.03, 0.011, rig.outside); }
