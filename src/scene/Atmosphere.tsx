...
export function Atmosphere() {
  ...
  const c = useMemo(() => ({ cold: new Color('#0d1119'), warm: new Color('#1b130d'), neutral: new Color('#0e1219'), out: new Color() }), []);
  useEffect(() => {
    scene.background = c.cold.clone();
    scene.fog = new FogExp2(c.cold.getHex(), 0.03);
  }, [scene, c]);
  let last = '';
  useFrame(() => {
    ...
    if (fog) { fog.color.copy(c.out); fog.density = lerp(0.026, 0.0095, rig.outside); }
